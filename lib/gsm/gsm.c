...
void handle_urc_qistate(gsm_t *gsm, const char *data, size_t len) {
    ...
    parse_string_quoted(&p, target->qistate.service_type, sizeof(target->qistate.service_type));
    parse_string_quoted(&p, target->qistate.remote_ip, sizeof(target->qistate.remote_ip));
    ...
    parse_string_quoted(&p, target->qistate.at_port, sizeof(target->qistate.at_port));
...
void handle_urc_qiurc(gsm_t *gsm, const char *data, size_t len) {
    ...
    if (strlen(type) == 0) {
        LOG_ERR("+QIURC 파싱 실패: type이 비어있음 (data=\"%.*s\")", (int)len, data);
    ...
    if (strcmp(type, "recv") == 0) {
        ...
        if (connect_id < GSM_TCP_MAX_SOCKETS) {
            ...
            if (xQueueSend(gsm->tcp.event_queue, &evt, pdMS_TO_TICKS(10)) != pdTRUE) {
                LOG_ERR("+QIURC: \"recv\" 이벤트 큐 오버플로우! (connect_id=%d)", connect_id);
            }
            else {}
        ...
        if (connect_id < GSM_TCP_MAX_SOCKETS) {
            tcp_event_t evt = {.type = TCP_EVT_CLOSED_NOTIFY, .connect_id = connect_id};

            if (xQueueSend(gsm->tcp.event_queue, &evt, pdMS_TO_TICKS(10)) != pdTRUE) {
                LOG_ERR("+QIURC: \"closed\" 이벤트 큐 오버플로우! (connect_id=%d)", connect_id);
            }
            else {}
        ...
        LOG_ERR("+QIURC 알 수 없는 type=\"%s\" (data=\"%.*s\")", type, (int)len, data);
    }
}
...
const urc_handler_entry_t urc_status_handlers[] = {
    {"RDY", handle_urc_rdy}, {"POWERED DOWN", handle_urc_powered_down}, {NULL, NULL}};
...
const urc_handler_entry_t urc_info_handlers[] = {
    ...
    {"+QIURC: ", handle_urc_qiurc},     {"+QCFG: ", handle_urc_qcfg}, {NULL, NULL}};
...
static inline size_t recv_payload_len(gsm_t *gsm) {
    return gsm->recv.len;
}
...
static void handle_urc_info(gsm_t *gsm) {
    ...
    LOG_DEBUG("handle_urc_info: recv.data=\"%s\" (len=%d)", gsm->recv.data, gsm->recv.len);
    ...
    while (entry->prefix != NULL) {
        ...
        if (strncmp(gsm->recv.data, resp, resp_len) == 0) {
            if (entry->handler) {
                ...
                size_t data_len = ((gsm->recv.len > resp_len) ? gsm->recv.len - resp_len : 0);


                if (gsm->cmd_mutex && xSemaphoreTake(gsm->cmd_mutex, portMAX_DELAY) == pdTRUE) {
 ...
 */
void gsm_send_at_cmd(gsm_t *gsm, gsm_cmd_t cmd, gsm_at_mode_t at_mode, const char *params,
...
void gsm_send_at_cgdcont(gsm_t *gsm, gsm_at_mode_t at_mode, gsm_pdp_context_t *ctx,
                         at_cmd_handler callback) {
    gsm_at_cmd_t msg = {
        .at_mode = at_mode,
        .cmd = GSM_CMD_CGDCONT,
    ...
    if (at_mode == GSM_AT_WRITE) {
        ...
        snprintf(msg.params, GSM_AT_CMD_PARAM_SIZE, "%d,\"%s\",\"%s\"", ctx->cid, pdp_type_str,
                 ctx->apn);
    }
    else {
        msg.params[0] = '\0';
...
static const gsm_hal_ops_t stm32_hal_ops = {.reset = gsm_port_reset, .send = gsm_uart_send};
...
void gsm_init(gsm_t *gsm, evt_handler_t handler, void *args) {
    ...
    gsm->producer_sem = xSemaphoreCreateBinary();
...
static void tcp_read_complete_callback(gsm_t *gsm, gsm_cmd_t cmd, void *msg, bool is_ok) {
...
void gsm_tcp_init(gsm_t *gsm) {
    ...
    xTaskCreate(gsm_tcp_task, "gsm_tcp", 1536, gsm, tskIDLE_PRIORITY + 3, &gsm->tcp.task_handle);
}
...
int gsm_tcp_open(gsm_t *gsm, uint8_t connect_id, uint8_t context_id, const char *remote_ip,
                 uint16_t remote_port, uint16_t local_port, tcp_recv_callback_t on_recv,
    ...
    snprintf(msg.params, GSM_AT_CMD_PARAM_SIZE, "%d,%d,\"TCP\",\"%s\",%d,%d,0", context_id,
             connect_id, remote_ip, remote_port, local_port);
...
int gsm_tcp_send(gsm_t *gsm, uint8_t connect_id, const uint8_t *data, size_t len,
                 at_cmd_handler callback) {
    if (!gsm || connect_id >= GSM_TCP_MAX_SOCKETS || !data || len == 0 || len > 1460) {
        return -1;
    }
...
int gsm_tcp_read(gsm_t *gsm, uint8_t connect_id, size_t max_len, at_cmd_handler callback) {
    if (!gsm || connect_id >= GSM_TCP_MAX_SOCKETS || max_len == 0 || max_len > 1500) {
        return -1;
    }
...
void gsm_send_at_qicfg_keepalive(gsm_t *gsm, uint8_t enable, uint16_t keepidle,
    ...
    snprintf(params, sizeof(params), "\"tcp/keepalive\",%d,%d,%d,%d", enable, keepidle,
