...
#define UNICORE_BIN_MSG_TABLE_SIZE \
    (sizeof(unicore_bin_msg_table) / sizeof(unicore_bin_msg_table[0]))
...
static const char *unicore_bin_msg_to_str(uint16_t msg_id) {
    switch (msg_id) {
#define X(name, id, handler, is_urc) \
    case id:                         \
        return #name;
        UNICORE_BIN_MSG_TABLE(X)
#undef X
...
static const char *unicore_resp_to_str(gps_unicore_resp_t resp) {
    if (resp == GPS_UNICORE_RESP_NONE)
        return "NONE";

    switch (resp) {
#define X(name, str)              \
    case GPS_UNICORE_RESP_##name: \
        return str;
...
parse_result_t unicore_ascii_try_parse(gps_t *gps, ringbuffer_t *rb) {
    ...
    /* 9. 명령어 응답 이벤트 핸들러 호출 */
    if (gps->handler) {
        gps_event_t event = {.type = GPS_EVENT_CMD_RESPONSE,
...
parse_result_t unicore_bin_try_parse(gps_t *gps, ringbuffer_t *rb) {
    ...
    LOG_DEBUG("Unicore Binary: %s (ID=%d, len=%d)", unicore_bin_msg_to_str(msg_id), msg_id,
              total_len);

    /* 11. 고수준 이벤트 핸들러 호출 (URC) */
    if (gps->handler) {
        gps_event_t event = {.protocol = GPS_PROTOCOL_UNICORE_BIN,
                             .timestamp_ms = xTaskGetTickCount(),
