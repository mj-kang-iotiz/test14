...
static void gsm_process_task(void *pvParameter) {
    ...
    // AT 커맨드 처리 태스크 생성
    xTaskCreate(gsm_at_cmd_process_task, "gsm_at_cmd", 1536, &gsm_handle, tskIDLE_PRIORITY + 2,
                NULL);
    ...
    while (1) {
        ...
        if (pos != old_pos) {
            if (pos > old_pos) {
                ...
                gsm_parse_process(&gsm_handle, &gsm_mem[old_pos], sizeof(gsm_mem) - old_pos);
                if (pos > 0) {
...
static void gsm_at_cmd_process_task(void *pvParameters) {
    ...
    while (1) {
        if (xQueueReceive(gsm->at_cmd_queue, &at_cmd, portMAX_DELAY) == pdTRUE) {
            ...
            gsm->ops->send(gsm->at_tbl[at_cmd.cmd].at_str, strlen(gsm->at_tbl[at_cmd.cmd].at_str));
