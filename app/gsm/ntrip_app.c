...
static int ntrip_build_http_request(char *buffer, size_t buffer_size) {
    ...
    int encoded_len = base64_encode(credentials, strlen(credentials), encoded_credentials,
                                    sizeof(encoded_credentials));
    ...
    int len = snprintf(buffer, buffer_size,
                       ...
                       "\r\n",
                       params->ntrip_mountpoint, encoded_credentials);
...
__attribute__((
    section(".ccmram"))) static char g_ntrip_http_request[512]; // 동적으로 생성된 HTTP 요청 저장
...
static int ntrip_connect_to_server(tcp_socket_t *sock) {
    ...
    while (retry_count < NTRIP_MAX_CONNECT_RETRY) {
        LOG_INFO("NTRIP 서버 연결 시도 [%d/%d]: %s:%d", retry_count + 1, NTRIP_MAX_CONNECT_RETRY,
                 params->ntrip_url, ntrip_port);
...
static void ntrip_gga_send_task(void *pvParameter) {
    ...
    while (1) {
        ...
        if (xQueueReceive(g_gga_send_queue, &gga_item, portMAX_DELAY) == pdTRUE) {
            ...
            if (ret > 0) {
                LOG_INFO("GGA 전송 완료 (%d bytes): %.*s", gga_item.len, gga_item.len,
                         gga_item.data);
            }
            else {
...
static void ntrip_tcp_recv_task(void *pvParameter) {
...

    {
...

        {
            ...
            LOG_ERR("GGA 전송 큐 생성 실패, 재시도 중... (%d/%d)", queue_retry,
                    NTRIP_INIT_MAX_RETRY);
    ...
    event_t ev = {.type = EVENT_NTRIP_CONNECTED, .data.ntrip = {.connected = true}};
...
void ntrip_task_create(gsm_t *gsm) {
    xTaskCreate(ntrip_tcp_recv_task, "ntrip_recv", 1536, gsm, tskIDLE_PRIORITY + 3,
                &g_ntrip_recv_task_handle);
}
