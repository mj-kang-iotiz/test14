...
static void lte_at_test_callback(gsm_t *gsm, gsm_cmd_t cmd, void *msg, bool is_ok);
static void lte_echo_off_callback(gsm_t *gsm, gsm_cmd_t cmd, void *msg, bool is_ok);
static void lte_cmee_set_callback(gsm_t *gsm, gsm_cmd_t cmd, void *msg, bool is_ok);
static void lte_qisde_off_callback(gsm_t *gsm, gsm_cmd_t cmd, void *msg, bool is_ok);
static void lte_airplane_ctrl_set_callback(gsm_t *gsm, gsm_cmd_t cmd, void *msg, bool is_ok);
static void lte_cpin_check_callback(gsm_t *gsm, gsm_cmd_t cmd, void *msg, bool is_ok);
static void lte_apn_set_callback(gsm_t *gsm, gsm_cmd_t cmd, void *msg, bool is_ok);
static void lte_apn_verify_callback(gsm_t *gsm, gsm_cmd_t cmd, void *msg, bool is_ok);
static void lte_network_check_callback(gsm_t *gsm, gsm_cmd_t cmd, void *msg, bool is_ok);
static void lte_keepalive_set_callback(gsm_t *gsm, gsm_cmd_t cmd, void *msg, bool is_ok);
...
void lte_init_start(void) {
    if (lte_init_state == LTE_INIT_IDLE) {
        LOG_INFO("LTE 초기화 시작 (시도 %d/%d)", lte_init_retry_count + 1, LTE_INIT_MAX_RETRY);
    }
    ...
    gsm_send_at_cmd(gsm_handle_ptr, GSM_CMD_AT, GSM_AT_EXECUTE, NULL, lte_at_test_callback);
}
...

{
    ...
    gsm_send_at_cmee(gsm, GSM_AT_WRITE, GSM_CMEE_ENABLE_VERBOSE, lte_cmee_set_callback);
}
...
static void lte_cpin_check_callback(gsm_t *gsm, gsm_cmd_t cmd, void *msg, bool is_ok) {
    ...
    gsm_pdp_context_t ctx = {.cid = 1, .type = GSM_PDP_TYPE_IP, .apn = "m2m-router.lguplus.co.kr"};
...
static void lte_apn_verify_callback(gsm_t *gsm, gsm_cmd_t cmd, void *msg, bool is_ok) {
    ...
    if (m && m->cgdcont.count > 0) {
        ...
        if (ctx->cid == 1 &&
            strcmp(ctx->apn, "m2m-router.lguplus.co.kr") == 0) { // internet.lguplus.co.kr
            LOG_INFO("APN 등록 완료: %s", ctx->apn);
            lte_init_state = LTE_INIT_NETWORK_CHECK;

            lte_network_check_count = 0;
            gsm_send_at_cmd(gsm, GSM_CMD_COPS, GSM_AT_READ, NULL, lte_network_check_callback);
            return;
        }
        else {
...
static void lte_network_check_callback(gsm_t *gsm, gsm_cmd_t cmd, void *msg, bool is_ok) {
    ...
    if (m && strlen(m->cops.oper) > 0) {
        LOG_INFO("apn 등록: %s (mode=%d, act=%d)", m->cops.oper, m->cops.mode, m->cops.act);
    ...
    // 네트워크 미등록 (재시도)
    LOG_INFO("네트워크 미등록 (%d/%d) 재시도...", lte_network_check_count,
...
void lte_network_check_timer_callback(TimerHandle_t xTimer) {
    ...
    gsm_send_at_cmd(gsm_handle_ptr, GSM_CMD_COPS, GSM_AT_READ, NULL, lte_network_check_callback);
}
...
static void lte_keepalive_set_callback(gsm_t *gsm, gsm_cmd_t cmd, void *msg, bool is_ok) {
    ...
    gsm_send_at_cmd(gsm, GSM_CMD_CPIN, GSM_AT_READ, NULL, lte_cpin_check_callback);
}
