...
static void lora_init_command_callback(bool success, void *user_data) {
    ...
    if (success) {
        // 명령어 성공
        LOG_INFO("LoRa init step %d/%d OK: %s", ctx->current_step + 1, ctx->cmd_count,
        ...
        lora_send_command_async(ctx->cmd_list[ctx->current_step], LORA_INIT_TIMEOUT_MS, 0,
        ...
        if (ctx->retry_count < LORA_INIT_MAX_RETRY) {
            // 재시도
            LOG_WARN("LoRa init step %d/%d failed, retrying (%d/%d): %s", ctx->current_step + 1,
                     ctx->cmd_count, ctx->retry_count, LORA_INIT_MAX_RETRY,
                     ctx->cmd_list[ctx->current_step]);
            ...
            // 최대 재시도 초과
            LOG_ERR("LoRa init failed at step %d/%d after %d retries: %s", ctx->current_step + 1,
                    ctx->cmd_count, LORA_INIT_MAX_RETRY, ctx->cmd_list[ctx->current_step]);
 ...
 */
static bool rtcm_reassembly_process(lora_rtcm_reassembly_t *reasm, const uint8_t *data,
                                    size_t len) {
    ...
    if (!reasm->has_header) {
        ...
        if (reasm->buffer_pos >= 3) {
            ...
            LOG_INFO("RTCM header parsed: payload=%d bytes, expected total=%d bytes", payload_len,
                     reasm->expected_len);

            // 예상 길이가 비정상적으로 크면 리셋
            if (reasm->expected_len > RTCM_REASSEMBLY_BUF_SIZE) {
                LOG_ERR("Invalid RTCM length: %d > %d - resetting", reasm->expected_len,
                        RTCM_REASSEMBLY_BUF_SIZE);
                rtcm_reassembly_reset(reasm);
                return false;
            }
        }
...
static bool lora_parse_p2p_recv(const char *data, lora_p2p_recv_data_t *recv_data) {
    ...
    LOG_INFO("P2P recv: RSSI=%d, SNR=%d, Len=%d", recv_data->rssi, recv_data->snr,
             recv_data->data_len);
    ...
    while (1) {
        if (xQueueReceive(instance.lora.cmd_queue, &cmd_req, portMAX_DELAY) == pdTRUE) {
            ...
            if (cmd_req.skip_response) {
                ...
                // 응답 대기 (타임아웃 적용)
                if (xSemaphoreTake(cmd_req.response_sem, pdMS_TO_TICKS(cmd_req.timeout_ms)) ==
                    pdTRUE) {
                    ...
                    if (cmd_req.is_async) {
                        ...
                        LOG_INFO("LoRa response received: %s", *(cmd_req.result) ? "OK" : "ERROR");
                    }
                    ...
                    if (cmd_req.toa_ms > 0) {
                        ...
                        if (elapsed_ms < cmd_req.toa_ms) {
                            ...
                            LOG_INFO("ToA already satisfied: elapsed=%dms >= ToA=%dms", elapsed_ms,
                                     cmd_req.toa_ms);
                        }
                    }
                }
    ...
    while (1) {
        ...
        if (pos != old_pos) {
            ...
            if (pos > old_pos) {
                ...
                if ((strstr(temp_buf, "at+recv=") || strstr(temp_buf, "AT+RECV=")) &&
                    ...
                    if (lora_parse_p2p_recv(temp_buf, &recv_data)) {
                        ...
                        if (instance.lora.p2p_recv_callback) {
                            instance.lora.p2p_recv_callback(&recv_data,
                                                            instance.lora.p2p_recv_user_data);
                        }
                        else {
                            ...
                            if (complete) {
                                ...
                                if (rtcm_validate_packet(
                                    ...
                                    // GPS UART로 직접 전송
                                    if (!gps_send_raw_data(
                                            GPS_ID_BASE, instance.lora.rtcm_reassembly.buffer,
                                            instance.lora.rtcm_reassembly.expected_len)) {
                                ...
                                if (instance.lora.rtcm_reassembly.buffer_pos >
                                    instance.lora.rtcm_reassembly.expected_len) {
                                    size_t remaining = instance.lora.rtcm_reassembly.buffer_pos -
                                                       instance.lora.rtcm_reassembly.expected_len;
                                    LOG_INFO("Remaining %d bytes in buffer - moving to front",
                                             remaining);

                                    // 남은 데이터를 버퍼 앞으로 이동
                                    memmove(
                                        instance.lora.rtcm_reassembly.buffer,
                                        &instance.lora.rtcm_reassembly
                                             .buffer[instance.lora.rtcm_reassembly.expected_len],
                                        remaining);
                ...
                else if ((strstr(temp_buf, "at+recv=") || strstr(temp_buf, "AT+RECV=")) &&
                         !instance.lora.init_complete) {
                            ...
                            if (complete) {
                                ...
                                if (instance.lora.rtcm_reassembly.buffer_pos >
                                    ...
                                    size_t remaining = instance.lora.rtcm_reassembly.buffer_pos -
                                                       instance.lora.rtcm_reassembly.expected_len;
                                    LOG_INFO(
                                        "Remaining %d bytes in buffer - moving to front (wrap)",
                                        remaining);
...
void lora_instance_init(void) {
    ...
    // RX Task 생성
    BaseType_t ret = xTaskCreate(lora_process_task, "lora_rx", 1024, NULL, tskIDLE_PRIORITY + 3,
                                 &instance.lora.rx_task);
    if (ret != pdPASS) {
    ...
    // TX Task 생성
    ret = xTaskCreate(lora_tx_task, "lora_tx", 1024, NULL, tskIDLE_PRIORITY + 3,
                      &instance.lora.tx_task);
    if (ret != pdPASS) {
    ...
    // 이벤트 처리 태스크 생성
    ret = xTaskCreate(lora_event_task, "lora_evt", 1024, NULL, tskIDLE_PRIORITY + 2,
                      &instance.event_task);
    if (ret != pdPASS) {
...
bool lora_send_command_async(const char *cmd, uint32_t timeout_ms, uint32_t toa_ms,
                             lora_cmd_callback_t callback, void *user_data, bool skip_response) {
    if (!instance.lora.initialized) {
        LOG_ERR("LoRa not initialized");
        return false;
    }
...
bool lora_set_p2p_config(uint32_t freq, uint8_t sf, uint8_t bw, uint8_t cr, uint16_t preamlen,
                         uint8_t pwr, uint32_t timeout_ms) {
    char cmd[128];
    snprintf(cmd, sizeof(cmd), "at+set_config=lorap2p:%lu:%d:%d:%d:%d:%d\r\n", freq, sf, bw, cr,
             preamlen, pwr);
    return lora_send_command_sync(cmd, timeout_ms);
}
...
void lora_app_start(void) {
    ...
    LOG_INFO("LoRa 앱 시작 - 모드: %s", config->lora_mode == LORA_MODE_BASE ? "BASE" : "ROVER");
...
bool lora_app_set_p2p_config(uint32_t freq, uint8_t sf, uint8_t bw, uint8_t cr, uint16_t preamlen,
                             uint8_t pwr, uint32_t timeout_ms) {
